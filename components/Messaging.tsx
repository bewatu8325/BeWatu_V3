        ...
        <div className="p-3 border-b" style={{ borderColor: '#e7e5e4' }}>
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-stone-50 border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-stone-300"
            />
          </div>
        </div>
...
const Messaging: React.FC<MessagingProps> = ({ users, messages, currentUser, onSendMessage, initialActiveUserId }) => {
  ...
  const sharedGoals = useMemo(() => {
    if (!activeChatPartner || !currentUser.professionalGoals || !activeChatPartner.professionalGoals) return [];
    return currentUser.professionalGoals.filter(goal => activeChatPartner.professionalGoals.includes(goal));
  ...
  // Available users to message (exclude self and already-conversing)
    ...
    <>
      {showNewMessage && (
        <NewMessagePicker
          ...
          onSelect={id => setActiveChatUserId(id)}
      ...
      <div className="bg-white rounded-2xl border overflow-hidden flex shadow-sm" style={{ borderColor: '#e7e5e4', height: 'calc(100svh - 9rem)' }}>
        {/* Conversation List */}
        <div
          className={`${showList ? 'flex' : 'hidden'} md:flex flex-col md:w-1/3 border-r w-full flex-shrink-0`}
          style={{ borderColor: '#e7e5e4' }}
        >
          {/* Header with New Message button */}
          <div className="flex items-center justify-between p-4 border-b flex-shrink-0" style={{ borderColor: '#e7e5e4' }}>
              ...
              <div className="flex-grow p-4 overflow-y-auto bg-stone-50 space-y-4">
                ...
                {messagesForActiveChat.map(msg => (
                  <div key={msg.id} className={`flex ${msg.senderId === currentUser.id ? 'justify-end' : 'justify-start'}`}>
                    <div
                      className={`max-w-[75vw] sm:max-w-xs md:max-w-md p-3 rounded-2xl ${msg.senderId === currentUser.id ? 'text-white' : 'bg-white text-stone-800 border'}`}
              ...
              <div className="p-4 border-t" style={{ borderColor: '#e7e5e4' }}>
                <form onSubmit={handleSendMessage} className="flex gap-2">
                  ...
                  <button
                    ...
                    className="border font-semibold px-3 py-2 rounded-xl hover:bg-stone-50 transition-colors disabled:opacity-50 flex items-center justify-center"
                  ...
                  <button
                    type="submit"
                    className="text-white font-semibold px-4 py-2 rounded-xl hover:opacity-90 transition disabled:opacity-50 flex-shrink-0"
                    style={{ backgroundColor: GREEN }}
