...
export default function ArenaIndustryView({
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      >
        {/* Sponsor branding */}
        {arenaData?.sponsorCompanyId && (
          <div className="absolute top-4 right-4 flex items-center gap-2 bg-white/80 rounded-full px-3 py-1.5 border border-white">
            {arenaData.sponsorLogoUrl ? (
              <img src={arenaData.sponsorLogoUrl} alt="" className="h-4 object-contain" />
            ) : (
              <Star size={12} style={{ color }} />
            )}
            <span className="text-xs font-medium text-stone-600">
              Presented by {arenaData.sponsorCompanyName}
            </span>
          </div>
        )}
