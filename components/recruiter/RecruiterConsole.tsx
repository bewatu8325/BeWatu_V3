  ...
  const NavItem: React.FC<{ label: string; view: RecruiterView }> = ({ label, view }) => (
    <button
      ...
      className={`px-3 py-2 text-sm font-semibold rounded-md transition-colors ${
        activeView === view ? 'bg-slate-700 text-cyan-300' : 'text-slate-400 hover:bg-slate-700/50'
      }`}
    >
      {label}
    </button>
          ...
          <>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                      isBlindMode ? 'bg-cyan-500' : 'bg-slate-600'
                    }`}
                  >
            ...
            <div className="mt-8">
                    ...
                    <div key={result.user.id}>
                      ...
                      >
                        <img
                          ...
                          alt="Candidate"
                          className={`w-20 h-20 rounded-full object-cover border-2 border-slate-600 ${isBlindMode ? 'filter grayscale' : ''}`}
                        />
              ...
              {!isSearching && searchResults.length === 0 && query && (
                <p className="text-center text-slate-400 py-8">No candidates found for your search.</p>
              )}
            </div>
          </>
        );
        ...
        <div className="container mx-auto px-4 sm:px-6 flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              onClick={onLogout}
              title="Logout"
              className="flex items-center space-x-2 text-slate-400 hover:text-cyan-400 p-2 rounded-lg focus:outline-none transition-colors"
            >
