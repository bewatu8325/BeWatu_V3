    ...
    <div className={`rounded-xl border transition-all ${expanded ? 'border-cyan-500/30 bg-slate-800/80' : 'border-slate-700 bg-slate-800/40 hover:border-slate-600'}`}>
      <div className="flex items-center gap-3 p-4 cursor-pointer" onClick={() => setExpanded(e => !e)}>
        {!isBlind && result.userAvatar ? (
          <img src={result.userAvatar} alt="" className="h-10 w-10 rounded-full object-cover border border-slate-700 shrink-0" />
        ) : (
          <div className="h-10 w-10 rounded-full bg-slate-700 flex items-center justify-center shrink-0 text-xs font-bold text-slate-400">
            {isBlind ? `C${idx + 1}` : result.userName?.[0] ?? '?'}
          </div>
        )}
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-slate-100">{displayName}</p>
          {!isBlind && <p className="text-xs text-slate-400 truncate">{result.userHeadline}</p>}
        </div>
      ...
      {expanded && (
        <div className="border-t border-slate-700 p-4 space-y-3">
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={label} className="flex items-center gap-3">
                ...
                <span className="text-xs font-bold w-8 text-right text-slate-300">{score}%</span>
              </div>
            ))}
          </div>
          ...
          {result.matchedValues.length > 0 && (
            <div>
              <p className="text-xs font-medium text-slate-400 mb-1.5">Shared values</p>
          ...
          {result.mismatches.length > 0 && (
            <div>
              <p className="text-xs font-medium text-slate-400 mb-1.5">Potential friction</p>
              <div className="flex flex-wrap gap-1.5">
  ...
  return (
    <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-4 space-y-4">
      <div className="flex items-center gap-2">
        <h3 className="text-sm font-bold text-slate-200">Your Culture Profile</h3>
        <Info className="h-3.5 w-3.5 text-slate-500" />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div>
          <label className="text-xs font-medium text-slate-400 mb-1 block">Collaboration style</label>
          <select value={profile.collaboration}
            onChange={e => onChange({ ...profile, collaboration: e.target.value })}
            className="w-full rounded-lg border border-slate-600 bg-slate-800 px-2.5 py-2 text-xs text-slate-200 focus:border-cyan-500 focus:outline-none">
        ...
        <div>
          <label className="text-xs font-medium text-slate-400 mb-1 block">Communication</label>
        ...
        <div>
          <label className="text-xs font-medium text-slate-400 mb-1 block">Work pace</label>
      ...
      <div>
        <label className="text-xs font-medium text-slate-400 mb-1.5 block">Company values (candidates matched against these)</label>
        <div className="flex flex-wrap gap-1.5 mb-2">
          {profile.values.map(v => (
            <span key={v} className="flex items-center gap-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 px-2.5 py-0.5 text-xs text-cyan-400">
              {v}
        ...
        <div className="flex gap-1.5">
          <input value={newValue} onChange={e => setNewValue(e.target.value)}
            ...
            className="flex-1 rounded-lg border border-slate-600 bg-slate-800 px-2.5 py-1.5 text-xs text-slate-200 placeholder:text-slate-500 focus:border-cyan-500 focus:outline-none" />
          <button onClick={addValue} disabled={!newValue.trim()}
            className="rounded-lg bg-slate-700 px-3 py-1.5 text-xs text-slate-200 hover:bg-slate-600 disabled:opacity-40 transition-colors">Add</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-xl font-bold text-slate-100">
            <Heart className="h-5 w-5 text-cyan-400" />Culture Fit Score
          </h1>
          <p className="mt-0.5 text-sm text-slate-400">Auto-matched from candidates' workStyle and values — no CV required.</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setIsBlind(b => !b)}
            className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${isBlind ? 'bg-slate-600 text-slate-200' : 'bg-slate-700/50 text-slate-400 hover:text-slate-200'}`}
          >
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <p className="text-xs text-slate-500">{results.length} candidates scored · ranked by fit</p>
